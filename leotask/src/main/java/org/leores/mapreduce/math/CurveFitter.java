package org.leores.mapreduce.math;
 ...
 * dkfz.de).
 * 2012-01-30: Modified for external Minimizer class and Calculateable fit by
 * Michael Schmid.
 ...
 * (3-parameter) fit types.
 * 2013-10-11: bugfixes, added setStatusAndEsc to show iterations and enable
 * abort by ESC
 */

public class CurveFitter extends Logger implements Calculateable {
	/** Constants for the built-in fit functions */
	public static final int STRAIGHT_LINE = 0, POLY2 = 1, POLY3 = 2, POLY4 = 3, EXPONENTIAL = 4, POWER = 5, LOG = 6, RODBARD = 7, GAMMA_VARIATE = 8, LOG2 = 9, RODBARD2 = 10, EXP_WITH_OFFSET = 11,
			GAUSSIAN = 12, EXP_RECOVERY = 13, INV_RODBARD = 14, EXP_REGRESSION = 15, POWER_REGRESSION = 16, POLY5 = 17, POLY6 = 18, POLY7 = 19, POLY8 = 20, GAUSSIAN_NOOFFSET = 21,
			EXP_RECOVERY_NOOFFSET = 22, CHAPMAN = 23;

	/** Nicer sequence of the built-in function types */
	public static final int[] sortedTypes = { STRAIGHT_LINE, POLY2, POLY3, POLY4, POLY5, POLY6, POLY7, POLY8, POWER, POWER_REGRESSION, EXPONENTIAL, EXP_REGRESSION, EXP_WITH_OFFSET, EXP_RECOVERY,
			EXP_RECOVERY_NOOFFSET, LOG, LOG2, GAUSSIAN, GAUSSIAN_NOOFFSET, RODBARD, RODBARD2, INV_RODBARD, GAMMA_VARIATE, CHAPMAN };

	/** Names of the built-in fit functions */
	public static final String[] fitList = { "Straight Line", "2nd Degree Polynomial", "3rd Degree Polynomial", "4th Degree Polynomial", "Exponential", "Power", "Log", "Rodbard", "Gamma Variate",
			"y = a+b*ln(x-c)", "Rodbard (NIH Image)", "Exponential with Offset", "Gaussian", "Exponential Recovery", "Inverse Rodbard", "Exponential (linear regression)", "Power (linear regression)",
			"5th Degree Polynomial", "6th Degree Polynomial", "7th Degree Polynomial", "8th Degree Polynomial", "Gaussian (no offset)", "Exponential Recovery (no offset)", "Chapman-Richards" }; // fList, doFit(), getNumParams() and makeInitialParamsAndVariations() must also be updated

	/** Equations of the built-in fit functions */
	public static final String[] fList = { "y = a+bx",
			"y = a+bx+cx^2", //STRAIGHT_LINE,POLY2
			"y = a+bx+cx^2+dx^3", "y = a+bx+cx^2+dx^3+ex^4", "y = a*exp(bx)", "y = a*x^b",
			"y = a*ln(bx)", //EXPONENTIAL,POWER,LOG
			"y = d+(a-d)/(1+(x/c)^b)",
			"y = b*(x-a)^c*exp(-(x-a)/d)", //RODBARD,GAMMA_VARIATE
			"y = a+b*ln(x-c)",
			"x = d+(a-d)/(1+(y/c)^b) [y = c*((x-a)/(d-x))^(1/b)]", //LOG2,RODBARD2
			"y = a*exp(-bx) + c",
			"y = a + (b-a)*exp(-(x-c)*(x-c)/(2*d*d))", //EXP_WITH_OFFSET,GAUSSIAN
			"y = a*(1-exp(-b*x)) + c",
			"y = c*((x-a)/(d-x))^(1/b)", //EXP_RECOVERY, INV_RODBARD
			"y = a*exp(bx)",
			"y = a*x^b", //EXP_REGRESSION, POWER_REGRESSION
			"y = a+bx+cx^2+dx^3+ex^4+fx^5", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6+hx^7", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6+hx^7+ix^8",
	...
	private Calculateable userFunction; // plugin with custom fit function
	...
	public void logSettings() {
		log("Simplex Fitting Options");
		log("Function name: " + getName() + "\n" + "Formula: " + getFormula());
		for (int i = 0; i < numParams; i++)
			log("p" + i + ":" + U.valToStr(initialParams[i], sPatNumOut));
		log("Maximum iterations:" + minimizer.getMaxIterations());
		log("Number of restarts:" + minimizer.getMaxRestarts());
		log("Error tolerance [1*10^(-x)]:" + (-(Math.log(maxRelError) / Math.log(10))));
	}

	/**
	 * Perform curve fitting with one of the built-in functions
	 * doFit(fitType, true) pops up a dialog allowing the user to set the
	 * initial
	...
	/**
	 * Fit a function defined as a evaltion String like
	 * "#$p(0)$+$p(1)$*$x$+$p(2)$*LOG($x$)#". Each $p(i)$ should only occur
	 * once. *
	 * 
	 * For good performance, it is advisable to set also the typical variation
	 * range of the initial parameters by the
	 * getMinimizer().setInitialParamVariations(double[]) method (especially if
	 * one or more of the initialParams are zero).
	 * Use getStatus() and/or getStatusString() to see whether fitting was
	 * (probably) successful and
	 * getParams() to access the result.
	 */
	public void doCustomFit(String formula, int numParams, double[] initialParams) {
		doCustomFit(null, formula, numParams, initialParams, null, false);
	}

	/**
	 * Fit a function defined in a user plugin implementing the Calculateable
	 * interface.
	 * 
	 * Use getStatus() and/or getStatusString() to see whether fitting was
	 * (probably) successful and getParams() to access the result.
	 * 
	 * @param userFunction
	 *            A plugin where the fit function is defined by the
	 *            calculate(params, x) method. This function must allow
	 *            simultaneous calls in multiple threads.
	 * @param numParams
	 ...
	 *            constructor (xData).
	 * @param showSettings
	 *            whether log initial parameters and a few numbers controlling
	 *            the minimizer.
	 */
	public void doCustomFit(Calculateable userFunction, int numParams, double[] initialParams) {
		doCustomFit(userFunction, null, numParams, initialParams, null, false);
	}
	 ...
	 * @param userFunction
	 *            A plugin where the fit function is defined by the
	 *            calculate(params, x) method.
	 *            This function must allow simultaneous calls in multiple
	 *            threads.
	 * @param numParams
	 *            Number of parameters of the fit function.
	 * @param formula
	 *            A String describing the fit formula, may be null.
	 * @param initialParams
	 *            Starting point for the parameters; may be null (than values
	 *            of 0 are used). The fit function with these parameters must
	 *            not return NaN for any of the data points given in the
	 *            constructor (xData).
	 * @param initialParamVariations
	 *            Each parameter is initially varied by up to +/- this value.
	 *            If not given (null), initial variations are taken as
	 *            10% of initial parameter value or 0.01 for parameters that are
	 *            zero.
	 *            When this array is given, all elements must be positive
	 *            (nonzero).
	 ...
	 *            the minimizer.
	 */
	public void doCustomFit(Calculateable userFunction, String formula, int numParams, double[] initialParams, double[] initialParamVariations, boolean showSettings) {
	...
	/**
	 * Sets the initial parameters, which override the default initial
	 * parameters.
	 */
	...
	/**
	 * Returns a reference to the Minimizer used, for accessing Minimizer
	 * methods directly.
	 ...
	 *            E.g. multiplyParams=1 if f(p0, p1, p2, p3...) can be expressed
	 *            as
	 *            p1*func(p0, p2, p3, ...) or p0 +p1*func(p0, p2, p3, ...) with
	 *            '0' being
	 *            the offsetparam.
	 * @param slopeParam
	 *            Index of a parameter that is multiplied with x and then summed
	 *            to the function.
	 *            E.g. '1' for f(p0, p1, p2, p3...) = p1*x + func(p0, p2, p3,
	 *            ...)
	 *            Only one, multiplyParam and slopeParam can be used (ie.e, the
	 *            other
	 *            should be set to -1)
	 */
	...
	public final double f(double[] p, double x) {
		...
		if (fitType != CUSTOM)
			...
			if (userFunction != null) { // function defined in plugin
				rtn = userFunction.calculate(p, x);
			} else if (customFormula != null) { // function defined in macro
				rtn = Double.parseDouble(U.eval("#" + customFormula + "#", this));
			}
		}
		return rtn;
	}
	...
	/**
	 * Returns value of built-in 'fitType' formula value for parameters "p" at
	 * "x"
	 */
	...
	/**
	 * Get the result of fitting, i.e. the set of parameter values for the best
	 * fit.
	 * Note that the array returned may have more elements than numParams;
	 * ignore the rest.
	 ...
	 * The residuals are with respect to the real data, also for fit types where
	 * the data are
	 * modified before fitting (power&exp fit by linear regression, 'Rodbard NIH
	 * Image' ).
	 * This is in contrast to sum of squared residuals, which is for the fit
	...
	/*
	 * Get the sum of the residuals (may be NaN if the minimizer could not start
	 * properly
	 * i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE).
	 */
	 ...
	 * Here, the standard deviation is defined here as the root-mean-square of
	 * the residuals
	 ...
	 * Approaches R^2 if the number of points is much larger than the number of
	 * fit parameters.
	 * For power, exp by linear regression and 'Rodbard NIH Image', this is
	 * calculated for the
	 * fit actually done, not for the residuals of the original data.
	...
	/**
	 * Get a short text with a short description of the status. Should be
	 * preferred over
	 * Minimizer.STATUS_STRING[getMinimizer().getStatus()] because
	 * getStatusString()
	 * better explains the problem in some cases of initialization failure (data
	 * not
	...
	/**
	 * Get a string with detailed description of the curve fitting results
	 * (several lines,
	...
	public String getResultString() {
		...
		resultS += "\nTime: " + time + " ms" + "\nSum of residuals squared: " + U.valToStr(getSumResidualsSqr(), sPatNumOut) + "\nStandard deviation: " + U.valToStr(getSD(), sPatNumOut) + "\nR^2: "
				+ U.valToStr(getRSquared(), sPatNumOut) + "\nParameters:";
		char pChar = 'a';
		double[] pVal = getParams();
		for (int i = 0; i < numParams; i++) {
			resultS += "\n	" + pChar + " = " + U.valToStr(pVal[i], sPatNumOut);
			pChar++;
		}
		return resultS;
	}
	...
	/**
	 * Return the result formula when the fitting is done using customized
	 * formula (via doCustomFit(String formula, int numParams, double[]
	 * initialParams) or doCustomFit(Calculateable userFunction, String formula,
	 * int numParams, double[] initialParams, double[] initialParamVariations,
	 * boolean showSettings) with a not null formula). When customFormula==null,
	 * the function returns null.
	 */
	public String getResultFormula() {
		String rtn = customFormula;
		if (rtn != null) {
			rtn = rtn.replace("$x$", "x");
			rtn = U.eval(rtn, this, sPatNumOut);
		}
		return rtn;
	}

	/**
	 * Set maximum number of simplex restarts to do. See
	 * Minimizer.setMaxRestarts for details.
	 */
	 ...
	 * 
	 * @param ijStatusString
	 *            Displayed in the beginning of the status message. No display
	 *            if null.
	 *            E.g. "Curve Fit: Iteration "
	 * @param checkEscape
	 *            When true, the Minimizer stops if escape is pressed and the
	 *            status
	 *            becomes ABORTED. Note that checking for ESC does not work in
	...
	/**
	 * Get maximum number of simplex restarts to do. See
	 * Minimizer.setMaxRestarts for details.
	 */
	...
	/**
	 * Returns the status of the Minimizer after doFit. Minimizer.SUCCESS
	 * indicates a
	 * successful completion. In case of Minimizer.INITIALIZATION_FAILURE,
	 * fitting could
	 ...
	 * problem.
	 * For further status codes indicating problems during fitting, see the
	 * status codes
	...
	/**
	 * Returns the code for a fit with given name as defined in fitList, or -1
	 * if not found
	 */
	...
	/**
	 * This function is called by the Minimizer and calculates the sum of
	 * squared
	 * residuals for given parameters.
	 * To improve the efficiency, simple linear dependencies are solved directly
	 * by linear regression; in that case the corresponding parameters are
	 * modified.
	 ...
	 */
	public final double calculate(double[] params, double dummy) {
	...
	/**
	 * For fits where linear regression is used to reduce the number of
	 * parameters handled
	 ...
	 * The Minimizer stores the sum of squared residuals directly behind its
	 * last parameter.
	 ...
	 * 'numParams',
	 * the offset and factor params (if any) are written to their proper
	 * positions in the
	 * params array
	 */
	private void doRegression(double[] params) {
		...
		if (offsetParam < 0) { // only 'factor' parameter, no offset:
			...
			sumResidualsSqr = sqr(factor) * sumX2 + numPoints * sqr(offset) + sumY2 + 2 * factor * offset * sumX - 2 * factor * sumXY - 2 * offset * sumY;
	...
	/**
	 * Convert full set of parameters to minimizer parameters and returns the
	 * sum of residuals squared.
	 * The last array elements, not used by the minimizer, are the offset and
	 * factor parameters (if any)
	 */
	private double fullParamsToMinimizerParams(double[] params) {
		...
		for (int i = 0, iNew = 0; i < numParams; i++)
			// leave only the parameters for the minimizer in the beginning of the array
	 ...
	 * Also checks for x values being non-negative for fit types that require
	 * this,
	...
	private boolean makeInitialParamsAndVariations(int fitType) {
		...
		if (!hasInitialParams) {
			...
			case EXP_RECOVERY: // a*(1-exp(-bx)) + c
				...
				initialParams[0] = (yMax - yMin) / Math.exp(initialParams[1] * xMean) * Math.signum(slope) * fitType == EXP_RECOVERY ? 1 : -1; // don't care, we will do this via regression
			...
			case CHAPMAN: // a*(1-exp(-b*x))^c
				...
				for (int i = 1; i < numPoints; i++)
					//find when we reach 50% of maximum
	...
	/**
	 * Set multiplyParams and offsetParam for built-in functions. This allows us
	 * to use linear
	 * regression, reducing the number of parameters used by the Minimizer by up
	 * to 2, and
	...
	private boolean isModifiedFitType(int fitType) {
		return fitType == POWER_REGRESSION || fitType == EXP_REGRESSION || fitType == RODBARD2 || fitType == GAUSSIAN;
	}
	...
	/**
	 * Get correct params and revert xData, yData if fit has been done via
	 * another function
	 */
	 ...
	 * @param maxRestarts
	 *            Number of restarts
	 * @param maxErrorCoefficient
	 *            Error tolerance [1*10^(-x)]
	 */
	private void settInitPars(double initialParams[], int maxIterations, int maxRestarts, int maxErrorCoefficient) {
		if (initialParamVariations == null)
			initialParamVariations = new double[numParams];
		if (initialParams != null) {
			for (int i = 0; i < numParams; i++) {
				double p = initialParams[i];
				if (!Double.isNaN(p)) {
		...
		if (maxRestarts >= 0)
			minimizer.setMaxRestarts(maxRestarts);
		setMaxError(Math.pow(10.0, -maxErrorCoefficient));
	}
