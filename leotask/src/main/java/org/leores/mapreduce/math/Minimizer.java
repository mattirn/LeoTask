 ...
 * 
 */
public class Minimizer extends Logger {
	...
	/**
	 * Status returned: Could not reinitialize the simplex because all attempts
	 * to find restarting
	 * parameters resulted in the target function returning NaN.
	 * Reinitialization is
	 * needed to obtain a reliable result; so the result may be inaccurate or
	 * wrong.
	 */
	...
	public final static String[] STATUS_STRING = { "Success", "Initialization failure; no result", "Aborted", "Re-initialization failure (inaccurate result?)",
			"Max. no. of iterations reached (inaccurate result?)", "Max. no. of restarts reached (inaccurate result?)" };
	...
	private Calculateable userFunction; // target function to minimize
	...
	private String sStatus = null; // shown together with iteration count in status, no display if null
	 ...
	 * new Hashtable<Thread, double[][]>(); //for each thread, holds a reference
	 * to its simplex
	 */
	...
	/**
	 * Set the the target function, i.e. function whose value should be
	 * minimized.
	 * 
	 * @param userFunction
	 *            The class having a function to minimize (implementing
	 *            the Calculateable interface).
	 ...
	 *            numParams+1 array
	 *            elements; the last one is needed to store the function value.
	 *            Further
	 *            array elements for private use in the user function may be
	 *            added by
	 ...
	 *            of the function.
	 */
	public void setFunction(Calculateable userFunction, int numParams) {
		if (maxIter <= 0) {
	...
	/**
	 * Perform minimization with the gradient-enhanced simplex method once or a
	 * few
	 * times, depending on the value of 'restarts'. Running it several times
	 * helps
	 * to reduce the probability of finding local minima or accepting one of the
	 * rare
	 * results where the minimizer has got stuck before finding the true
	 * minimum.
	 * We are using two threads and terminate after two equal results. Thus,
	 * apart
	 ...
	 *            this value. If not given (null), initial variations are taken
	 *            as
	 *            10% of initial parameter value or 0.01 for parameters that are
	 *            zero.
	 *            When this array is given, all elements must be positive
	 *            (nonzero).
	 *            If one or several initial parameters are zero, is advisable to
	 *            set the initialParamVariations
	 *            array to useful values indicating the typical order of
	 *            magnitude of the parameters.
	 *            For target functions with only one minimum, convergence is
	 *            fastest with large values of
	 *            initialParamVariations, so that the expected value is within
	 *            initialParam+/-initialParamVariations.
	 *            If local minima can occur, it is best to use a value close to
	 *            the expected global minimum,
	 *            and rather small initialParamVariations, much lower than the
	 *            distance to the nearest local
	 *            minimum.
	 * 
	 ...
	 * @param initialParamVariations
	 *            Parameters (variables) are initially varied by up to +/-
	 *            this value. If not given (null), iniital variations are taken
	 *            as
	 *            10% of inial parameter value or 0.01 for parameters that are
	 *            zero.
	 *            When this array is given, all elements must be positive
	 *            (nonzero).
	 *            If one or several initial parameters are zero, is advisable to
	 ...
	 *            minimum.
	 * 
	 * @return status code; SUCCESS if it is considered likely that a minimum of
	 *         the
	 ...
	 * from the best corner of the simplex. Note that the array returned may
	 * have more
	 * elements than numParams; ignore the rest.
	 * May return an array with only NaN values in case the minimize call has
	 * returned
	 * an INITIALIZATION_FAILURE status or that abort() has been called at the
	 * very
	...
	/**
	 * Get the value of the minimum, i.e. the value associated with the
	 ...
	 * needs
	 * between one and numParams+3 calls of the target function (typically two
	 * calls
	 * per iteration)
	 */
	 ...
	 * threads).
	 * The number of function calls will be higher, up to about twice the number
	 * of
	 * iterations.
	 * Note that the number of iterations needed typically scales with the
	 * square of
	 * the dimensions (i.e., numParams^2).
	 * Default value is 1000 * numParams^2 (half that value if maxRestarts=0),
	 * which is
	 * enough for >99% of all cases (if the maximum number of restarts is set to
	 * 2);
	...
	/**
	 * Get maximum number of iterations allowed. Unless given by
	 * 'setMaxIterations',
	 ...
	 * With n>0, two threads are used, and if the two results do not agree
	 * within
	 * the error bounds, additional optimizations are done up to n times, each
	 * with two threads. In any case, if the two best results are within the
	 * error
	 ...
	 * problems,
	 * while greatly reducing the risk of running into spurious local minima or
	 * non-
	 * optimal results due to the minimizer getting stuck. In problematic cases,
	 * the
	 * improved
	 * The 'n' value does not refer to the restarts within one minimization run
	 * (there, at least one restart is performed, and restart is repeated until
	 * the result
	 * does not change within the error bounds).
	 * This value does not affect the 'minimizeOnce' function call.
	 * When setting the maximum number of restarts to a value much higher than
	 * 3, remember
	...
	/**
	 * Get number of minimizations completed (i.e. not aborted or stopped
	 * because the
	 * number of minimization was exceeded). After a minimize(..) call,
	 * typically 2
	 * for unproblematic cases. Higher number indicate a functin that is
	 * difficult to
	...
	/**
	 * Set the resolution of the parameters, for problems where the target
	 * function is not smooth
	 * but suffers from numerical noise. If all parameters of all vertices are
	 * closer to the
	 * best value than the respective resolution value, minimization is
	...
	/**
	 * Aborts minimization. Calls to getParams() will return the best solution
	 * found so far.
	 ...
	 * "<sStatus> 50 (max 750); ESC to stop"
	 * 
	 * @param sStatus
	 ...
	 */
	public void setStatusAndEsc(String statusString, boolean checkEscape) {
		this.sStatus = statusString;
	...
	/**
	 * Get the full simplex of the current thread. This may be useful if the
	 * target function
	...
	/**
	 * One minimization run (including reinitializations of the simplex until
	...
	private void minimizeOnce(double[] initialParams, double[] initialParamVariations, int seed) {
		...
		while (status == SUCCESS || status == REINITIALIZATION_FAILURE) {
			double[] paramVariations = makeNewParamVariations(simp, bestVertexNumber, initialParams, initialParamVariations);
	...
	private int minimize(double[][] simp) {
		...
		while (true) {
			...
			if (status != SUCCESS)
				break;
			if ((sStatus != null || checkEscape) && totalNumIter > nextIterationForStatus) {
				...
				if (time - startTime > 1000L) { // display status and check for ESC after the first second					
					if (sStatus != null) {
						log(sStatus + totalNumIter + " (" + maxIter + " max)");
					}
				}
			}
		}
	 ...
	 * worstVertex
	 * The new point is written to'newVertex' and target function is evaluated
	 * there
	 */
		...
		if (initialParams != null) {
			for (int i = 0; i < numParams; i++)
				if (Double.isNaN(initialParams[i]))
					log(LOG_WARNING, "Initial Parameter[" + i + "] is NaN");
			System.arraycopy(initialParams, 0, simp[0], 0, Math.min(initialParams.length, numParams));
		}
		evaluate(simp[0]);
		if (Double.isNaN(value(simp[0]))) {
			log(LOG_WARNING, "Initial Parameters yield NaN:" + sVertex(simp[0]));
			findValidInitalParams(simp[0], initialParamVariations, random);
		}
		if (Double.isNaN(value(simp[0]))) {
			log(LOG_ERROR, "Could not find initial parameters not yielding NaN:");
			return null;
		}
		if (initializeSimplex(simp, initialParamVariations, random))
			return simp;
		else {
			log(LOG_ERROR, "Could not make simplex vertices not yielding NaN" + sSimplex(simp));
			return null;
		}
	}
	...
	/**
	 * Whether the distance between two vertices is less than the resolution of
	 * the parameters
	 */
	...
	/**
	 * Reinitialize an existing simplex: Create new vertices around the best
	 * one, keeping the rough size of
	...
	private boolean initializeSimplex(double[][] simp, double[] paramVariations, Random random) {
		...
		for (int i = 0; i < numParams; i++) {
			double range = paramVariations != null && i < paramVariations.length ? paramVariations[i] : 0.1 * Math.abs(simp[0][i]); // if nothing else given, based on initial parameter
			...
			do { //try finding a vertex that does not yield NaN
				...
				if (numTries < maxAttempts / 3) // (don't orthogonalize if finding valid params was very difficult
					...
					for (int v2 = 1; v2 < v; v2++) { // to avoid a degenerate simplex, make orthogonal to all others
						...
						for (int i = 0; i < numParams; i++)
							//subtract component parallel to the other vector
	 ...
	 * Note that the simplex should have adapted in size to the useful range of
	 * each
	 * parameter, but in special cases it might have become degenerate [i.e. the
	 * (hyper-)volume spanned by the vertices might be zero; in such a case it
	 * may
	 * happen that one parameter has the same value for all vertices of the
	 * simplex].
	 * 
	 * For each parameter, as a variation value we use the typical difference
	 * w.r.t. the best
	 * vertex of the simplex.
	 * To avoid a degenerate simplex, we have to know what the the relative
	 * orders of magnitude of
	 * the parameter variations should be. If initialParamVariations are given,
	 * we use them as
	 * indication of the typoical relative values; otherwise we use the
	 * parameter values themselves
	 * and assume that the variation of each parameter should be roughly the
	 * same fraction of the value.
	 * If the variation of a parameter in the simplex w.r.t. this estimated
	 * variation is much
	 * smaller (<10^3) than typical for the parameters, we increase the
	...
	private double[] makeNewParamVariations(double[][] simp, int bestVertexNumber, double[] initialParams, double[] initialParamVariations) {
		...
		for (int i = 0; i < numParams; i++) { // for all parameters
			...
			relatedTo[i] = initialParamVariations != null && initialParamVariations.length >= numParams ? initialParamVariations[i] : // relate to initialParamVariations (if given),
					Math.max(Math.abs(initialParams[i]), Math.abs(simp[bestVertexNumber][i]));
			double logRelativeVariation = paramVariations[i] > relatedTo[i] ? 0 : Math.log(paramVariations[i] / relatedTo[i]);
	...
	private void evaluate(double[] vertex) {
		vertex[numParams] = userFunction.calculate(vertex, 0);
	}
	...
	//simplex [Iteration: s0(p1, p2....), s1(),....]
	private synchronized String sSimplex(double[][] simp) {
		String rtn = "";
		for (int i = 0; i < numVertices; i++)
			rtn += sVertex(simp[i]) + " | ";
		return rtn;
	...
	private synchronized String sVertex(double[] vertex) {
		String rtn = "";
		for (int j = 0; j < numParams; j++)
			rtn += "  " + U.valToStr(vertex[j], sPatNumOut);
		rtn += " -> " + U.valToStr(value(vertex), sPatNumOut);
		return rtn;
	}
}