...
*/
package org.leores.mapreduce.math.rand;

import org.leores.mapreduce.math.Arithmetic;
import org.leores.mapreduce.math.Probability;
 ...
 * Rejection/Inversion method. This is a port of <A HREF=
 * "http://wwwinfo.cern.ch/asd/lhc++/clhep/manual/RefGuide/Random/RandBinomial.html"
...
public class Binomial extends AbstractDiscreteDistribution {
	...
	public Binomial(RandomEngine randomGenerator) {
		...
		for (;;) {
			...
			if ((Km = Math.abs(K - m)) <= DMAX_KM || Km + Km + 2L >= ss) {
				...
				if (V <= T + E) {
					...
					if (V <= ch + (n + 1.0) * Math.log((double) nm / (double) nK) + (K + 0.5) * Math.log(nK * pq / (K + 1.0)) - Arithmetic.stirlingCorrection(K + 1)
							- Arithmetic.stirlingCorrection(nK))
						break;
				}
			}
		}
	...
	/**
	 * Luc Devroye's "Second Waiting Time Method" on page 522 of his text
	 * "Non-Uniform Random Variate Generation." This function works fast
	 * when n*p is small. It also returns random binomial numbers with correct
	 * distribution even when n*p is small. When n*p<1E-2 Acceptance
	 * Rejection/Inversion in
	 * generateBinomial(int n, double p) fails to generate numbers with right
	 * distribution!
	 * 
	 * @author leoking
	 * @param n
	 ...
	 * @return
	 */
	public int generateBinomialS(int n, double p) {
		if (p > 0.5) {
			return n - generateBinomialS(n, 1 - p);
		}
		double log_q = Math.log(1.0 - p);
		int x = 0;
		double sum = 0;
		while (true) {
			sum += Math.log(randomGenerator.nextDouble()) / (n - x);
			if (sum < log_q) {
				break;
			}
			x++;
	...
	/**
	 * Returns a random number from the distribution using Acceptance Rejection/Inversion method.
	 * This function is fast but can not generate number with right distribution when n*p<1E-2.
	 */
	public int nextInt() {
	...
	/**
	 * Returns a random number from the distribution using Luc Devroye's
	 * "Second Waiting Time Method".
	 * This function works rigth and fast when n*p is small.
	 * @author leoking
	 * @return
	 */
	public int nextIntS() {
		return generateBinomialS(n, p);
	}
