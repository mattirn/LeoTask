...
import org.leores.mapreduce.util.PersistentObject;
...
public abstract class RandomEngine extends PersistentObject {
	 ...
	 * This has the effect that random engines can now be used as function
	...
	public static RandomEngine makeDefault(long seed) {
		return new MersenneTwisterFast(seed);
	}

	/**
	 * Returns a random double in the half-open range from [0.0,1.0). Thus 0.0
	 * is a valid result but 1.0 is not. <br>
	 * Note: this is different from the original Colt definition which
	 * excludes 0.0.
	 */
	 ...
	 * is a valid result but 1.0f is not. <br>
	 * Note: this is different from the original Colt definition which
	 * excludes 0.0.
	 */
	 ...
	 * interval <code>(0.0,1.0)</code> (excluding 0.0 and 1.0). raw() is used in
	 * many other Colt classes.
	 */
	public double raw() {
		int nextInt;
