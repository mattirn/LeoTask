 ...
 * as well as <tt>float</tt>'s and <tt>double</tt>'s in the open unit intervals
 * <tt>(0.0f,1.0f)</tt> and <tt>(0.0,1.0)</tt>, respectively.
 * The seed can be any 32-bit integer except <tt>0</tt>. Shawn J. Cokus
 * commented that perhaps the seed should preferably be odd.
 * <p>
 ...
 * one subinterval. <tt>double</tt>'s are formed by dividing the interval
...
public class MersenneTwister extends RandomEngine {
	 ...
	 * sequences.
	 * After this call has returned, the copy and the receiver have equal but
	 * separate state.
	 * 
